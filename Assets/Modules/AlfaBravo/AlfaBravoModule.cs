...
public class AlfaBravoModule : MonoBehaviour {
	...
	public void RandomizeLetters() {
		...
		Debug.LogFormat("[Alfa-Bravo #{0}] New digit on small display is {1}", moduleId, stage.character);
	...
	private void SetLetterToExpected(int index, char expected) {
		...
		if (index == 2) {
			...
			if (remaingMinutes > 0) {
				...
				Debug.LogFormat("[Alfa-Bravo #{0}] Add {1} delay minutes to letter #3", moduleId, reserveTime);
		...
		Debug.LogFormat("[Alfa-Bravo #{0}] Setting letter #{1} to '{2}'", moduleId, index + 1, newCharacter);
